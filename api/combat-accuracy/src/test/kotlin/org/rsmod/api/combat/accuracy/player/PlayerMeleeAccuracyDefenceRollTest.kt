...
class PlayerMeleeAccuracyDefenceRollTest {
    ...
    fun `calculate base defence roll with loadout`(loadout: Loadout) {
        ...
        val effectiveDefence =
            PlayerMeleeAccuracy.calculateEffectiveDefence(
        ...
        val defenceRoll =
            PlayerMeleeAccuracy.calculateBaseDefenceRoll(effectiveDefence, defenceBonus)
    ...
    data class Loadout(
        ...
        fun withArmourBonus(bonus: Double) = copy(armourBonus = bonus)
    }
    ...
    private object LoadoutProvider : TestArgsProvider {
        override fun args(): List<TestArgs> =
            testArgsOfSingleParam(
                ...
                // This scenario is currently bugged in the wiki dps calc. Wearing Torag's set
                // without Amulet of the damned gives a `51436` roll, but equipping Amulet of
                // the damned incorrectly lowers it to `3732` due to the "missing hp" multiplier
                // that is meant to increase the effective defence level.
                Loadout(expectedDefenceRoll = 53592)
                    .withDefensiveStyle()
                    .withSmellingSalts()
                    .withPiety()
                    .withDefenceBonus(bonus = 244)
                    .withArmourBonus(bonus = 1.05),
                /* Loadout Group */
