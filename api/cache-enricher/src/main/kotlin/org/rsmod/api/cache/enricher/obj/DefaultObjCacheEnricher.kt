...
constructor(
    ...
    private fun ObjPluginBuilder.putSpot(anim: Int?, paramType: ParamSpot) {
        val id = anim ?: return
        val spot = spotTypes.getValue(id)
        param[paramType] = spot.toHashedType()
    }
    ...
    private fun ObjPluginBuilder.putProjAnim(projAnimName: String?) {
        if (projAnimName == null) {
            return
        }
        val symbolId =
...
private data class ExternalObjConfig(
    ...
    val projectileLaunchDouble: Int?,
    val speed: Int?,
    val range: Int?,
