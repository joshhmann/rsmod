 ...
 *
 * DSL quick-reference: `add(Drop(objs.bones))` — guaranteed drop `10 weight Drop(objs.coins, 25)` —
 * weighted item drop, qty fixed `5 weight Drop(objs.feather, 5..15)` — weighted item drop, qty
 * range `1 weight Drop(objs.nothing)` — nothing slot (reduces loot rate) `1 outOf 128 chance
 * Drop(...)` — tertiary 1-in-128 chance
 */
@Deprecated(
    message = "Deprecated in favor of npc-drops/tables/*. Use BasicF2pDropTables + per-NPC tables.",
    level = DeprecationLevel.WARNING,
)
class F2pDropTables @Inject constructor(private val registry: NpcDropTableRegistry) :
    PluginScript() {

    override fun ScriptContext.startup() {
        // Legacy DTX-based tables kept for reference only. Do not register to avoid duplicates.
    }

    // ─────────────────────────────────────────────────────────────────────────

    private fun registerGoblin() =
        registry.registerDtx(
            npcType = npcs.goblin,
            label = "Goblin",
    ...
    private fun registerGreaterDemon() =
        registry.registerDtx(
