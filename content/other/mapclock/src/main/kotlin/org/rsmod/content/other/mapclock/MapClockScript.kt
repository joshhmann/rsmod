...
class MapClockScript @Inject constructor(private val clock: MapClock) : PluginScript() {
    ...
    private var Player.msPastMinute by intVarBit(clock_varbits.date_ms_past_minute)
    private var Player.secsPastMinute by intVarBit(clock_varbits.date_secs_past_minute)

    override fun ScriptContext.startup() {
        onPlayerLogin { player.initClockTimer() }
        onPlayerSoftTimer(clock_timers.playtime_clock) { player.incrementClock() }
    }

    private fun Player.initClockTimer() {
        softTimer(clock_timers.playtime_clock, 1)
    }
