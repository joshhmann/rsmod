...
class Flynn @Inject constructor(private val shops: Shops) : PluginScript() {
    override fun ScriptContext.startup() {
        onOpNpc1(falador_npcs.flynn) { shopDialogue(it.npc) }
        onOpNpc3(falador_npcs.flynn) { player.openShop(it.npc) }
    }
    ...
    private suspend fun ProtectedAccess.shopDialogue(npc: Npc) =
        startDialogue(npc) { flynnDialogue(npc) }

    private suspend fun Dialogue.flynnDialogue(npc: Npc) {
        chatNpc(happy, "Hello there! Want to buy a mace? I've got all types!")
