...
class SquireScript @Inject constructor(private val objRepo: ObjRepository) : PluginScript() {
    ...
    private suspend fun Dialogue.squireOfferQuest() {
        ...
        chatNpc(sad, "Oh, hello. Im in such trouble...)
        val choice =
            choice3(
                Whats wrong?",
                1,
        ...
        when (choice) {
            1 -> {
                chatPlayer(quiz, "Whats wrong?)
                chatNpc(sad, Ive lost Sir Vyvins ceremonial sword!)
                ...
                val helpChoice =
                    choice2(
                        ...
                        4,
                        Thats too bad. Good luck with that!",
                        5,
                    )
                when (helpChoice) {
                    4 -> {
                        ...
                        chatNpc(happy, "Please, Im counting on you!)
                        access.setQuestStage(QuestList.knights_sword, 1)
                    }
                    5 -> {
                        chatPlayer(neutral, Thats too bad. Good luck with that!")
                        chatNpc(sad, "*sniff* Im doomed...)
                    }
                }
            }
            2 -> {
                ...
                chatNpc(neutral, Im just a squire, so I dont have a plume yet.)
            }
    ...
    private suspend fun Dialogue.squireInProgress() {
        ...
        if (hasSword) {
            ...
            val option =
                choice2(
                    Im working on it. Any leads?",
                    1,
                    "Not yet. Ill keep looking.,
                    2,
                )
            when (option) {
                1 -> {
                    chatPlayer(quiz, Im working on it. Any leads?")
                    chatNpc(neutral, "I heard Thurgo lives south of Port Sarim, near the Ice Dungeon.")
                    chatNpc(neutral, "Hes an Imcando dwarf - a brilliant smith if you can find him.)
                }
                2 -> {
                    chatPlayer(neutral, Not yet. Ill keep looking.")
    ...
    private suspend fun Dialogue.completeSwordReturn() {
        ...
        chatNpc(happy, "Sir Vyvin will never know it was lost! Youve saved my life!)
        chatNpc(happy, Thank you! Thank you so much!)
        chatPlayer(happy, The Imcando craftsmanship is remarkable. Youd never tell it apart!")
    ...
    private suspend fun Dialogue.squireCompleted() {
        chatNpc(happy, "Hello again! Sir Vyvin still hasnt noticed the difference!)
        chatNpc(happy, Youre a lifesaver. If you ever need anything, just ask!")
