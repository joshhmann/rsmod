...
class Ranael @Inject constructor(private val shops: Shops) : PluginScript() {
    override fun ScriptContext.startup() {
        onOpNpc1(al_kharid_npcs.ranael) { shopDialogue(it.npc) }
        onOpNpc3(al_kharid_npcs.ranael) { player.openShop(it.npc) }
    }
    ...
    private suspend fun ProtectedAccess.shopDialogue(npc: Npc) =
        startDialogue(npc) { ranaelDialogue(npc) }

    private suspend fun Dialogue.ranaelDialogue(npc: Npc) {
        chatNpc(happy, "Hello. Would you like to buy some armour?")
