# Sandbox-to-CT175 Handoff Contract

## Purpose

Structured handoff package for cross-profile kanban delegation. Any worker that cannot directly execute on CT 175 produces a handoff that Mai can apply, validate, and commit on CT 175. This makes cross-profile delegation safe and auditable.

## When to Use

- A **tai/rei/nei** profile worker needs to make code changes to the rsmod project
- A worker has been dispatched to a content task but lacks SSH access to CT 175
...
target_host: "ct175"              # Always ct175
...
# Deployment
sync_pattern: "scp-sandbox"       # sandbox -> SCP -> CT 175
...
2. Copy files/ to CT 175 targets
3. Run validation commands
4. If validation passes: commit
5. Update docs
6. Set completion level to CT175_VALIDATED or COMMITTED
```
...
|:----------|:--------------|:------------|
| Worker generates code but can't reach CT 175 | Create handoff package, set status to `SANDBOX_STAGED` | Receive handoff, SCP files, compile, validate, commit, set status to `COMMITTED` |
| Worker finds a blocker during sandbox work | Block with `kanban_block(reason="...")` | Review and unblock when resolved |
| Mai finds compile error on CT 175 | N/A (worker already done) | Report failure, do NOT commit, return task to ready for fix |
...
| **CT175_APPLIED** | Mai copied files to CT 175 target paths | Mai | Files on CT 175 |
| **CT175_VALIDATED** | Compile + raw-ID pass on CT 175 | Mai | All validations green |
| **COMMITTED** | Changes committed to git on CT 175 | Mai | git log confirms |
